...
export function DrawingToolbar() {
  const [active, setActive] = useState('Crosshair');
      ...
      <div className="rounded-xl bg-[#0b1014] border border-[rgba(212,168,83,0.2)] p-2 flex-1 space-y-1 overflow-y-auto">
        {tools.map((tool) => (
          <button
            ...
            className={`w-full h-9 px-2 rounded-md flex items-center gap-2 text-xs ${active === tool ? 'bg-[rgba(212,168,83,0.18)] text-[#f6a400] border border-[rgba(212,168,83,0.35)]' : 'text-[#c8ced6] hover:bg-[#10151b]'}`}
          >
